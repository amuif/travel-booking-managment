            ...
            <div className="mx-auto flex w-full items-end gap-3 space-x-0.5 rounded-xl border-none p-4 shadow-lg md:w-[90%]">
                <div className="h-full w-1/2">
                    ...
                    <Input
                        ...
                        onChange={(e) => {
                            setDestination((prevState) => ({ ...prevState, to: e.target.value }));
                        }}
                        aria-label="destination input"
                        className="h-16 border-none focus:border-none"
                    />
                </div>
                ...
                <div className="w-full md:w-1/5">
                    <Button
                        onClick={() => {
                            ...
                            window.location.href = `/flights?from=${destination.from.replace(/\s+/g, '_')}&to=${destination.to.replace(/\s+/g, '_')}&adults=${1}`;
                        }}
                        className="bg-primary flex cursor-pointer items-center justify-end px-10 py-8 text-xl text-white md:text-3xl"
                    >
                        Explore
                    </Button>
                </div>{' '}
